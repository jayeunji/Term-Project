...
<a href="All_order_list.jsp"> 모든 주문 내역 보러가기 </a> <br>

<table class="table table-bordered">

<a href="take.jsp"> 총 매출액 확인하기 </a> <br> <br>

<tr><th> 주문 번호 <th> 품목 번호 <th> 주문 수량 <th> 상품 가격 </th></tr>
<%
	Class.forName("oracle.jdbc.OracleDriver");
	
	Connection conn = DriverManager.getConnection("jdbc:oracle:thin:@203.247.166.94:1521:xe","DB201813084","201813084");
	Statement st = conn.createStatement();
	ResultSet rs = st.executeQuery("select * from orders natural join order_items natural join products where status = 'Shipped'");
	
	while (rs.next()){
		%> <tr> <td> <%=rs.getInt("order_id") %>
		<td> <%=rs.getInt("product_id") %>
		<td> <%=rs.getInt("quantity") %>
		<td> $ <%=rs.getInt("list_price") %>
