...
<tr><th> 주문 번호 <th> 품목 번호 <th> 창고 재고 수량 <th> 재고 창고 이름</th></tr>
<%
	Class.forName("oracle.jdbc.OracleDriver");

	Connection conn = DriverManager.getConnection("jdbc:oracle:thin:@203.247.166.94:1521:xe","DB201813084","201813084");
	Statement st = conn.createStatement();	
	ResultSet rs = st.executeQuery("select * from orders natural join order_items natural join inventories natural join warehouses where status = 'Pending' and quantity < i_quantity order by order_id");
	...
	while (rs.next()){
		...
		<td> <%=rs.getInt("i_quantity") %>
		<td> <%=rs.getString("warehouse_name") %>
